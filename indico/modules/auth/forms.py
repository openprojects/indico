...
def _check_signin(form, field):
    signal_response = values_from_signal(signals.users.handle_signin.send(field.data), single_value=True, as_list=True)
    if signal_response:
        signin_allowed, message = signal_response[0]
        if not signin_allowed:
